...
- Phase: **Phase 1 in progress** — Step 2 (auth + account) DONE + Step 3 catalogue *pages* DONE 2026-07-09 (e2e 18/18 + 25/25). **Next: Step 4 (booking flow)**; real-timetable *seeding* is the only Step 3 remainder, blocked on Q6 (Jasmine) — prod /sessions shows "timetable being finalised" until then. kit_list column added to mem_offerings (20260709 migration). Business-rule values live as constants in `src/lib/business-rules.ts` (built). shadcn deferred — brand-token primitives in `components/ui/form.tsx`; add shadcn at Step 3+ only if dialog/table/calendar need it. Live at https://members.empowrcic.org (Netlify site `76f903e4-3795-406a-9478-34be6b0ed015`). **Deploys via GitHub push → CI**; `publish = ".next"` — CI resolves it relative to base (never CLI-deploy).
