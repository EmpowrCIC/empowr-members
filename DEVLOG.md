...
## 2026-09-02 (session 2) — Every session page 404'd on production for ~10 hours; the cause was the 09-01 fix applied to 3 of 4 reads
...
- **🔑 `notFound()` POISONS THE CACHE PERMANENTLY; A THROW DOES NOT.** Next stores a not-found render in the durable cache and keeps serving it — proven with cache-busting query strings still returning `Cache-Status: "Next.js"; hit` at a TTL **34,000 seconds past expiry**. **This is why cache-clearing alone did not hold:** a cache-cleared rebuild restored all 9 pages at 10:56, and they were re-poisoned within 30 minutes, because a single transient failure during the post-deploy render storm is enough. Only deploying the throw actually fixed it. A thrown error is never cached, so the next request retries.
- **Fixed in PR #14 (`279d6b0`), deployed cache-cleared. 9/9 pages 200, inactive and unknown slugs still 404, renamed slug still 308.**
...
- **`verify:catalogue` pins it, and the tests that matter are STRUCTURAL.** The behavioural tests would not have caught this bug. The two that would assert that every query routes through `unwrap` and that no `if (error)` block returns instead of throwing. **Both were proven to fail by reintroducing the original bug** (`4 queries but 3 unwrap() calls`), not just observed passing.
