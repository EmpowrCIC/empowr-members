...
## 2026-09-04 - PR #33 reviewed and merged; the verify suites now run on every pull request

- Merged #33 (signup, waiver and emergency-contact clarity) after review found one blocking defect: the Brevo signup sync was awaited **after** the participant insert with no time limit. The existing try/catch bounds *failure*, not *silence* - a hung Brevo would time the function out, hide the 201, and a retry would duplicate the household member. Bounded via `BrevoClient`'s injectable fetch rather than inside `call()`, which the nightly reconciliation shares for paged bulk reads
- **Emergency contact is now required for every skater, adults included.** This reverses the 2026-08-18 change that narrowed it to minors; the team re-took the decision deliberately on safeguarding grounds. 8 of 22 existing participants have no contact recorded and cannot be edited until one is added - they can still book and skate
- Added `verify:emergency-contact` (4 tests). That rule has now flipped twice, and the only thing carrying the reasoning between flips was a comment the reversing PR deleted. Proved the gate trips by restoring the permissive schema: 2 failures, exit 1
- Added `.github/workflows/verify.yml` - 10 suites, 75 tests, plus typecheck - and made `verify` a required status check. Nothing had ever run them automatically; #33 reached mergeable without a single test executing
- **The suites cannot run on the deploy's Node 20.** They are `.ts` executed by `node --test`, and native type stripping needs Node 22+; they passed locally only because dev machines run 24. CI pins 22 - closing that split (bump `netlify.toml`, or add a stripper) is a separate deploy decision
- `npm run lint` is deprecated and prompts interactively with no ESLint config, so it cannot run unattended - excluded from CI until repaired
