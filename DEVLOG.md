...
- **Four plans seeded from the KB**, all `active=false`, each entitling exactly one offering: Skate Jam £25, Sk8 Skool Kidz £30, All Ages £40, SYNKRON8 £45. Four matching test-mode Stripe Prices created. **No live-mode Prices yet** — deliberate.
...
- **Still open before any live subscription:** Customer Portal configuration (no API — MCP exposes only GET for portal configurations), the `rk_` key needs Products-read and Subscriptions-write (no API exists for key scopes), live-mode Prices, and entitlement questions **Q3-Q7** which gate Steps 4-6. Q4 (family coverage) is the only one that changes the schema — `mem_memberships` has no `participant_id`.
- Two labelled test customers remain in Stripe test mode (`cus_V93vGlq1a7yvIF`, `cus_V94ez4e4CoQRkS`) — no delete operation is exposed via MCP; remove from the dashboard.
