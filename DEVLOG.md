...
- **The register already resolved subscriber cover live** (`coverForOccurrence()` → `registerSubscribers()`), so Step 4 was NOT about making subscribers visible — that already worked. It was about the two things that key off an actual `mem_bookings` row and cannot be satisfied by a live read: `mem_hold_bookings()` only counts booking rows for capacity, so a session could look full to staff while the RPC kept selling paid places past it; and there was no check-in control for a subscriber, which also meant Phase 3 attendance/revenue reporting would never see them.
- **`lib/materialize-member-bookings.ts` — reconciliation, not a delta.** A participant can hold more than one Subscription (Sk8 Skool Kidz Monday *and* Wednesday), so "cancel this membership's bookings" can't be computed from the one membership that just changed without risking cancelling a still-covered slot's rows too. `reconcileMemberBookings(participantId)` instead recomputes that participant's entitled occurrences from ALL their currently-active memberships, creates what's missing (`source='member'`, £0, `confirmed`), and cancels future (not-yet-attended) rows no longer covered. Idempotent by construction — safe on a Stripe webhook retry or a re-run cron tick.
- **Reused `slotCoversOccurrence()` rather than reimplementing slot matching in SQL.** That function exists specifically to keep the Europe/London BST trap solved in exactly one tested place — a second, subtly different implementation on the database side is the same class of bug this project has already shipped three times (`checkWaivers`, `coverForOccurrence`). Confirmed the existing BST test suite still passes unchanged (`verify:subscriptions`, 25/25).
- **Two trigger points, deliberately different jobs.** The Stripe webhook calls `reconcileMemberBookings()` for just that participant right after every membership upsert — catches subscribe/cancel/past_due immediately, best-effort (logged, not thrown — the membership status write is the part Stripe retries on failure). A new daily Netlify scheduled function, `netlify/functions/materialize-member-bookings.ts` (03:15 UTC), calls `reconcileAllMemberBookings()` across every participant who has ever held a membership — the only thing that catches an occurrence added to a slot *after* someone already subscribed to it, which reacting to that participant's own membership change alone cannot. First scheduled function in this project; mirrors PecuvateDashboard's `nightly-inventory` convention (`[functions]` block in `netlify.toml`, cron in the function's own `config` export). Added `@netlify/functions` as a dependency.
- **A real regression caught before it shipped: materialising a row would have silently dropped the lapsed-waiver warning.** Today's "No waiver — do not let them take part" badge only exists because the live-only subscribers list runs `checkWaivers()` per render. A `source='member'` booking row bypasses that entirely — nobody visits the booking page, so nothing gates it. Fixed by extending `RegisterRow` with a `waiverSigned` field, live-checked for `member` rows only (`online`/`walk_in` stay implicitly true — both already gated at creation), via a `resolveSignedParticipants()` helper factored out of the near-identical code `registerSubscribers()` already had. The register's check-in column now shows the same warning instead of a check-in button when a materialised subscriber's waiver has lapsed.
- **Capacity and the "over capacity" banner needed no code changes.** Once a subscriber's place is a `confirmed` row, `mem_hold_bookings()`'s existing count (`pending_payment`/`confirmed`/`attended`, already fixed 09-01 to include `attended`) naturally shrinks around them — the paid-booking capacity check was always correct, it just couldn't see subscribers before. `register-summary.ts`'s arithmetic collapses to the simple case for anyone materialised, since they move out of the separate `subscribers` count into `confirmed`.
- **The old "Subscribers" section is now a transient fallback, reframed as such.** It used to be the everyday way a subscriber appeared (permanently, since Step 4 didn't exist). Copy changed from "There is no check-in button for them" to "haven't been added to the list above yet — that happens automatically, usually within a day," and the heading changed to "Recently subscribed." It still degrades gracefully: `registerSubscribers()` already excludes anyone with a live booking row, so it empties out on its own as materialisation catches up — no risk of it disagreeing with the main table.
- **Deliberately did NOT add age-eligibility or capacity checks to the materialisation insert.** Q5's answer was that a Subscription reserves a place unconditionally; the register's existing over-capacity banner is the accepted UI for what happens when that collides with a full room, and age eligibility isn't checked by `coverForOccurrence()` today either — not a gap this task introduced, and fixing it belongs with Step 1's entitlement gate, not here.
- **Not yet verified live.** No real subscription has gone through Stripe against the deployed webhook (same open gate as Step 3) — `npx tsc --noEmit` is clean, `npm run build` succeeds, and `verify:register` (6/6) and `verify:subscriptions` (25/25) both still pass unchanged, but nothing here has proven a row actually lands from a real event yet.
- `planning/phases/phase-2/CONTEXT.md` Step 4 row updated; Step 1's row was also stale (still said Q3/Q5/Q6/Q8 open — they closed 2026-08-31) and corrected in the same pass.
