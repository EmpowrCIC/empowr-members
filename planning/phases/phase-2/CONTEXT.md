...
| Step | State |
|---|---|
| 1 — Entitlement gate | ✅ **DONE.** All of Q1-Q8 closed (Q1/Q2 by the KB 2026-08-26; Q3/Q5/Q6/Q8 by Empowr 2026-08-31) — see [entitlement-intake.md](./entitlement-intake.md). This row previously said Q3/Q5/Q6/Q8 were still open; that was stale. |
...
| 4 — Entitled booking path | ⚠️ **Built, NOT yet verified live.** The refusal half shipped 2026-09-02 (`1f0f3a8`). The positive half — materialising £0 `source='member'` booking rows so a subscriber has capacity, a live-checked waiver gate and a check-in control — is now built: `lib/materialize-member-bookings.ts` (`reconcileMemberBookings`/`reconcileAllMemberBookings`), called synchronously from the Stripe webhook on every membership change and daily via the new `netlify/functions/materialize-member-bookings.ts` scheduled function (03:15 UTC) as a catch-up sweep for occurrences added after someone already subscribed. Register UI updated: member-source rows show a "Subscribed" badge and a live waiver check in place of the check-in button when unsigned; the old "Subscribers" section is now framed as the (self-healing) gap before materialisation runs, not the everyday case. **Still needs one real subscription end to end** to prove a row actually lands, same gate as Step 3. |
